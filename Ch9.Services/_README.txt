Project containing the Service classes. 
...
The task of this component is to deserialize the Json response (wraped into subclasses of TmdbResponseBase) from the Data access layer into Model objects to consume by clients.

The resulting Model objects are wraped into the derivates of ApiCommunicationServiceResponseBase base class which also signals success status for the higher level components.
...
Since the WebApi server sends in some cases poorly typed JSON responses, in some cases deserialization contains custom logic and error handling
...
with the values deserialized into actual Model objects. This is highly flexible and also compatible with Xamarin.Forms AppDictionary
...
Privide configuration services for freshly created Model objects based on server settings. 
...
-query information about available movie videos from Youtube (title, length, availability, available media streams)
